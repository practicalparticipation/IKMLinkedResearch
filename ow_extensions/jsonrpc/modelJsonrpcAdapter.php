...
class modelJsonrpcAdapter
{
    ...
    /**
     * @desc exports a model as rdf/json
     * old@desc exports a model as rdf/xml 
     * @param string modelIri
     * @return string
     */
    public function export($modelIri)
    {
	//return $this->store->exportRdf($modelIri); //rdf/xml NOTE broken in ontowiki 0.9.6
        return $this->store->exportRdf($modelIri, 'turtle'); //turtle now default
	//return $this->store->exportRdf($modelIri, 'n3'); //Notation 3 rapper 1.4.21 fails to parse these files although they are valid
	//return $this->store->exportRdf($modelIri, 'json'); //RDF/JSON (Talis) NOTE this is 4 times faster than either Turtle or Notation 3, but not suppoerted by rapper to parse. 
    }
