...
class CsvimportController extends OntoWiki_Controller_Component
{
    public function init()
    {
        // init component
        parent::init();
	$this->view->headScript()->appendFile($this->_componentUrlBase . 'scripts/urlEncode.js');
	$this->view->headScript()->appendFile($this->_componentUrlBase . 'scripts/jquery.scrollTo-1.4.2.js');
	$this->view->headScript()->appendFile($this->_componentUrlBase . 'scripts/jquery.sparql.js');
...
//var_dump($configurations);
    ...
    protected function saveconfigAction(){
        ...
        if( $this->_createBaseModel() ){
            ...
            $fp = fopen('extensions/csvimport/configs/'.$name.'.cfg', 'w');
            fwrite($fp, $val);
            fclose($fp);
