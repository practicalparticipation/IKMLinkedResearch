...
class Statement extends Object {
  ...
  function Statement($subj, $pred, $obj) {
	...
	if (!is_a($pred, 'Resource') || is_a($pred, 'BlankNode')) {
		$errmsg = RDFAPI_ERROR . 
		          '(class: Statement; method: new): Resource expected as predicate, no blank node allowed. S:'.$subj . ' P:' . $pred . ' O:'. $obj;
		trigger_error($errmsg, E_USER_ERROR); 
	}
	if (!(is_a($obj, 'Resource') or is_a($obj, 'Literal'))) {
		$errmsg = RDFAPI_ERROR . 
		          '(class: Statement; method: new): Resource or Literal expected as object. S:'.$subj . ' P:' . $pred . ' O:'. $obj;
		trigger_error($errmsg, E_USER_ERROR); 
	}
